...
From the `palmerpenguins` package, data courtesy of Dr. Kristen Gorman
...
-   Comment représenter ces données en perdant le moins d'information
    possible? [Visualisation]{.bleu}
-   Toutes les variables sont-elles utiles ? [Compression]{.bleu}
-   Peut-on mettre en évidence des liens entre variables ?
    [Compression]{.bleu}
-   Peut-on mettre en évidence des liens entre individus ?
    [Compression]{.bleu}

[Point de vue biologique]{.rouge}

-   Quelles sont les différences morpholgiques entre espèces, entre sexe
    et entre les îles ?
...
-   Trouvant une (ou plusieurs) représentations en 2 dimensions la plus
    fidèle possible,
-   Faisant apparaître les liens plus complexes entre variables
    ($X_3 = X_1 + X_2$)
-   étant capable d'identifier les individus qui se ressemblent
    globalement
:::
...
Toutes les méthodes d'analyse factorielle organisent les données dans un
tableau composé de [n individus]{.alea} (lignes) et de [p
...
-   Si une caractéristique est quantitative, alors on note dans une
    colonne la valeur mesurée sur l'individu.
-   Si une caractéristique $j$ est qualitative,
    -   soit on note sa modalité dans la colonne $j$,
    -   soit on utilise la représentation *one hot encoding*, qui
        consiste à représenter cette caractéristique qualitative sur
        autant de colonnes que de modalités et attribuer un $1$ dans la
        colonne correspondant à la modalité de l'individu et 0 aux
        autres.
...
-   island : l'île sur laquelle le mancho a été observé (Biscoe, Dream,
    Torgersen)
...
-   Quantifier la quantité d'information disponible ou perdue par une
    représentation

2.  Comprendre quelles sont les informations redondantes (variables
    liées),
3.  Idéalement des individus éloignés dans le nuage initial, restent
...
ggplot() +
  ...
  # Ajouter des labels mathématiques pour les distances
  # geom_text(aes(x = 2.5, y = 0.2), label = expression(x[ik] - bar(x)[k]), size = 5) +
  geom_text(aes(x = 2.5, y = 0.2), label = as.character(expression(x[i]^k)), parse = TRUE , size = 5, col = "#F7A913") +
  ...
  geom_text(aes(x = 0.3, y = 2), label = as.character(expression(R^n)), parse = TRUE, ,col="#398d55", size = 5) +
  
  # Ajouter l'origine O = G
  geom_text(data = origins, aes(x = x, y = y, label = label), vjust = -0.8, size = 5) +
  
  # Ajouter variable k 
  geom_text(data = vark, aes(x = x, y = y), label = as.character(expression(ind[i])), parse = TRUE,  col="#398d55", vjust = -0.8, size = 5) +
...
-   Ces deux méthodes ont beaucoup de points communs et de nombreuses
    variantes intéressantes
-   Comprendre finement l'ACP permet de bien comprendre toutes les
    extensions suivantes
-   La mise en oeuvre requiert un peu de pratique car il faut faire des
    choix avisés.
...
-   Trouver une (ou plusieurs) représentations en 2 dimensions la plus
    fidèle possible,
...
-   une mesure de la quantité d'information présente dans un jeu de
    données,
...
-   des données bien rangées dans un tableau $\Xbf$ où
    ...
    -   une colonne $j$ à une variable $X_{ij}$ la valeur de la variable
        $j$ mesurée sur l'individu $i$.
...
-   Analyse en composantes principales ACP
    -   Un exemple d'ACP\
    -   Comprendre les mécanismes derrière l'ACP (notion d'inertie,
        distance entre individus, changement de repères, la correlation
        comme un cosinus)
-   Analyse Factorielle multiple AFM : une extension de l'ACP pour tenir
    compte de groupes de variables
-   Analyse Factorielle Discriminante AFD : une extension de l'ACP pour
    tenir compte de groupes d'individus
-   Analyse Factorielle des Correspondances AFC
-   Analyse des Correspondances multiples ACM : lorsqu'on a plus que
