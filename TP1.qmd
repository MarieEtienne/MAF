...
sum_dta <- walking_dta |>
  ...
  mutate(across(is.numeric,  \(x) num(x,digits = 4)))
...
```{r}
sum_dta  |> summarise(Inertie = sum(variance))
```
...
4.  On souhaite utiliser la distance euclidienne puis réaliser une ACP.
    le cours indique qu'on diagonalise la matrice $M^{1/2}VM^{1/2}$. Indiquer ce que
...
5.  On diagonalise la matrice $M^{1/2}VM^{1/2}$ et on obtient les valeurs propres
...
6.  Si on sqouhaite réaliser une ACP normée, on change de métrique,  quelle est l'inertie portée par l'axe de vecteur directeur   $(1,0,0,0)^\top$ dans ce cas ?
