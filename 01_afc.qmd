...
[Question : est ce que les différentes espèces sont présentes de la même manière sur les différentes îles ?]{.rouge}
...
L'analyse factorielle des correspondances (AFC) est une méthode factorielle pour l'exploration statistique d'une [table de contingence]{.rouge} définie par [deux]{.rouge} variables [qualitatives]{.rouge}.

[Objectif : Le but est d'étudier la liaison entre ces deux variables qualitatives]{.rouge .large}(notamment quelles associations de modalités sont sur-représentées).
...
* et l'ensemble des colonnes du tableau.
...
$${\boldsymbol{X}}^\top {\boldsymbol{X}}= \begin{pmatrix} \class{orange}{C_1} & \class{vert}{K} \\ \class{vert}{K^\top} & \class{rouge}{C_2}\end{pmatrix},$$
...
-   l'hypothèse alternative est $H_1$: $A$ et $B$ ne sont pas
    indépendantes.
...
-   $\frac{f_{ij}}{f_{i+}}$ est la fréquence d'apparition de la modalité
    $j$ au sein de la population $A=i$. Si $A$ et $B$ sont
    indépendantes, alors $\frac{f_{ij}}{f_{i+}}= f_{+j}$

-   $\frac{f_{ij}}{f_{+j}}$ est la fréquence d'apparition de la modalité
    $i$au sein de la population $B=j$. Si $A$ et $B$ sont indépendantes,
    alors $\frac{f_{ij}}{f_{+j}}= f_{i+}$
...
$$G_I = \sum_{i=1}^I f_{i+} x_i = (f_{+1}, \ldots, f_{+J})$$ On a le
nuage de points $X$, complété par les poids $f_{i+}.$

### La métrique

La distance dont on munit l'espace ${\mathbb{R}}^J$ consiste à donner un
poids $1/f_{+j}$ à la j$^\text{ème}$ dimension, la distance au carré
(dite distance du $\chi^2$) entre les points $i_1$ et $i_2$ est définie
par

$$ d_{\chi^2}^2(x_{i_1}, x_{i_2}) = \sum_{j=1}^J \frac{1}{f_{+j}}\left (\frac{f_{i_1j}}{f_{i_1+}} -\frac{f_{i_2j}}{f_{i_2+}} \right)^2$$

Ce choix est motivé par l'inertie du point $x_i$ par rapport au centre
de garivité $G_I$, en effet
...
L'inertie mesure l'écart à l'indépendance. L'étude du nuage $N_I$ rend
compte de la structure des données.
::::

## Profils-colonnes

Dans le tableau de contingence, lignes et colonnes jouent des rôles
symétriques
...
$$G_J = \sum_{j=1}^J f_{+j}\, x^j = (f_{1+}, \ldots, f_{I+})$$ On a le
nuage de points $X$, complété par les poids $f_{+j}.$
...
- L'inertie projetée sur l'axe $\ell$ est données par l'inertie du nuage projeté sur cet axe.
    ...
    - concernant les profils lignes
        $\sum_{i}^I f_{i+} \lVert OH_i^{\ell}\rVert^2 = \lambda_{\ell}$
    - concernant les profils colonnes $\sum_{j}^J f_{+j} \lVert OH_j^{\ell}\rVert^2 = \lambda_{\ell}$
...
La valeur propre $1$ signifie que donc une association parfaite entre
ligne et colonnes.
...
fviz_ca(res.ca, axes = 2:3, invisible = c("row.sup"), select.row =  list(cos2 = 10))
```
...
pour des données qualitatives, avec seulement deux variables
qualitatives
...
On fait deux ACPS : \* une ACP sur les profils lignes affcetés du poids
de sa marge avec la métrique 1/ marge de la colonne \* une ACP sur les
profils colonnes affcetés du poids de leur marge avec la métrique 1/
marge de la ligne

la métrique est choisi pour que l'inertie totale soit le coefficient phi
du test de chi2 (chi2/n)

C'est une mesure d'écart à l'independance

Une des forces de l'ACP est la représentation jointe de slignes et des
colonnes grâce aux relations barycenytriques

les profils lignes ayant de svaleur positives sur un axe sont associés
préférentiellement aux colonnes ayant des valeurs, tandis qu'ils
s'associent moins qu'attendu avec les colonnes qui ont des valeurs
négatives

Les contributions, des lignes/ et des colonnes ssont intéressantes. Il
faut vérifier la qualité d ela représentaion
