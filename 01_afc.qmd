...
#| results: hold
#| message: false
res.ca <- CA(deces_dta, row.sup = 66:nrow(deces_dta), graph = FALSE)
...
tibble(marge_row = res.ca$call$marge.row, Cause = row.names(deces_dta[1:65,]))  |> ggplot() +  aes(x=Cause, y =marge_row ) +
  geom_bar(stat="identity") + ylab("Marge sur les lignes") +
  coord_flip()
