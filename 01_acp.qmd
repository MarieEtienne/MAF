...
L'inertie (terme hérité de la physique) *par rapport à un point*
...
Le meilleur résumé du nuage de points se résumant à un point est le
...
$$I_{\boldsymbol{g}} = I = \frac{1}{n}  \sum_{i=1}^n     \left( \sum_{k=1}^p \left( x_{i}^k - x_{\bullet}^k  \right)^2 \right)  = \sum_{k=1}^p  \left( \frac{1}{n}  \sum_{i=1}^n  \left( x_{i}^k - x_{\bullet}^k  \right)^2 \right)  = \sum_{k=1}^p  Var(\boldsymbol{x}^{k})$$
...
-   $X^\top W X$ est [symétrique]{.rouge} et [définie positive]{.rouge}
    donc [diagonalisable]{.rouge}.

-   Il existe une matrice $P$ unitaire $P^\top P = I_p,$ et $D$ une
    matrice diagonale telle que $$X^\top W X = P D P^\top.$$
...
$X^\top W X$. (la matrice de covariance).
...
Dans ce cas l'inertie totale est le nombre de variables (voir remarque
@sec-inertie)
...
matrice $X^\top M W X,$ il s'agit de la matrice de corrélation.
:::
...
-   Elles sont centrées, de variance
    $\lambda_k=\sum_{i=1}^n p_i c_{ik}^2$ et non-corrélées 2 à 2.
-   Les $C^k$ sont vecteurs propres de $XX^\top$ de valeur propre
    $\lambda_k$.
:::
...
or $C^\top C$ est diagonale avec $C_k^k = n lambda_k$, donc
$C^\top C^k=(0, \dots, n\lambda_k, 0, \ldots)$ et
...
* Dans le cas de l'ACP normée , la matrice $V$ est la matrice de covariance et $M = Diag(s_k^{-1})$,
