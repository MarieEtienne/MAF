...
<!-- [Comment visualiser au mieux ces données pour faire apparaître les liens entre variables et identifier des ressemblances entre individus ?]{.rouge} -->
...
$$I_{\boldsymbol{g}} = I = \frac{1}{n}  \sum_{i=1}^n     \left( \sum_{k=1}^p \left( x_{i}^k - x_{\bullet}^k  \right)^2 \right)  = \sum_{k=1}^p  \left( \frac{1}{n}  \sum_{i=1}^n  \left( x_{i}^k - x_{\bullet}^k  \right)^2 \right)  = \sum_{k=1}^p  Var(\xcol{k})$$
...
Dans le cadre de l'ACP, on veut construire une ensemble d'axes orthogonaux (une nouvelle base de $\R^p$), de façon à ce que 
...
$\frac{1}{n} X^\top X$ est la matrice de covariance, on écrit souvent $\frac{1}{n} X^\top X$ sous la forme
$X^\top W X$ ($W$ est une matrice diagonale, dont chaque terme diagonal vaut $1/n$, $W_{ii}$ est le poids attribué à l'observation $i$).

$X^\top W X$ est [symétrique]{.rouge} et [définie positive]{.rouge} donc [diagonalisable]{.rouge}. 

Il existe une matrice $P$ unitaire $P^\top P = I_p,$ et $D$ une matrice diagonale telle que 
$$X^\top W X = P D P^\top.$$

Les termes diagonaux de $D$ sont les [valeurs propres, toutes positives, ]{.rouge} de $X^\topW X$ rangées par odre décroissant, $\lambda_1 > \lambda_1 \ldots >\lambda_p$ et  $U$ est la matrice des vecteurs propres associés.
...
$\boldsymbol{b}$ est une base orthonormale, la base formée par les vecteurs propres de $X^\top X$.
:::
...
En se plaçant dans la nouvelle base, posons $\boldsymbol{v_1} = P^\top \boldsymbol{u_1},$  $\norm{\boldsymbol{v_1}}=1$:
...
[$\boldsymbol{u_1}$ est le premier vecteur propre]{.rouge} de $X^\top W X$. (la matrice de covariance).
...
[$\boldsymbol{u_2}$ est le deuxième vecteur propre]{.rouge} de  de $X^\top W X$.
...
* Lorsque l'on résume  le nuage à sa projection sur un [axe factoriel]{.rouge} $\boldsymbol{u_k}$, on ne garde que l'information associée à cet axe soit $I_{\boldsymbol{u_k}^\perp}=\lambda_k$.
...
$$\frac{\lambda_k}{\sum_{k=1}^p\lambda_{k_1} +\lambda_{k_2}}$$
...
La contribution à la distance entre individu de la variable  `Mass`est plus importante que celle de la variable `bill_length_mm`.
...
Dans ce cas l'inertie totale est le nombre de variables (voir remarque @sec-inertie) 


Lorsque l'on veut s'abstraire des unités, on centre te on réduit les données pour réaliser une [ACP normée]{.rouge}.
...
:::: {.callout-note icon="false" appearance="minimal"}
### Definition (métrique)
Une métrique $M$ permet de définir une distance entre les observations (ainsi, M est une matrice de $\R^p\times\R^p$ symétrique, définie et positive).

:::

### Calcul de distance

Soient $\xbf_1 \in \R^p$ et $\xbf_é\in\R^p$, la distance entre $\xbf_1$ et $\xbf_2$pour la métrique $M$ est
$$d_M(\xbf_1, \xbf_2)^2= \norm{\xbf_1 - \xbf_2}_M^2 = (\xbf_1 - \xbf_2)^\top M (\xbf_1 - \xbf_2).$$
...
### Exemple $I_p$
 ...
 $$d_M(\xbf_1, \xbf_2)^2= \norm{\xbf_1 - \xbf_2}_M^2 = (\xbf_1 - \xbf_2)^\top I_p (\xbf_1 - \xbf_2) =\sum_{k=1}^p (x_{1k}-x_{2k})^2.$$
...
$$d_M(\xbf_1, \xbf_2)^2=  (\xbf_1 - \xbf_2)^\top M (\xbf_1 - \xbf_2) =\sum_{k=1}^p \frac{(x_{1k}-x_{2k})^2}{s_k^2}=\sum_{k=1}^p \left ( \frac{x_{1k}}{s_k}-\frac{x_{2k}}{s_k} \right)^2.$$
...
Les coordonnées  des $n$ observations sur les  $p$ axes factoriels sont groupées dans la matrice $C$: 
$c_{ik}$ est la coordonnée de l'observation $i$ sur l'axe $\Delta_{\boldsymbol{u_k}}$. La  $k$-ème composante principale est la colonne $k$ de la matrice $C$, notée $C^k$.
$$
...
\begin{align}
<\class{alea}{x_i}, \boldsymbol{u_1}> &= \norm{\class{alea}{x_i}} \norm{\boldsymbol{u_1}} cos(\theta_i) \\
& = \class{alea}{x_i}^\top \boldsymbol{u_1}, \quad \left( \text{ou} \class{alea}{x_i}^\top M \boldsymbol{u_1}  \text{avec une autre métrique \right) \\
\end{align}

$$\cos^2(\theta_i) = \frac{\class{alea}{x_i}^\top \boldsymbol{u_1}}{\norm{\class{alea}{x_i}}}$$
:::

::::

[Interprétation}{.rouge}: plus $\cos^2(\theta_i)$ est proche de $1$, mieux l'individu est représenté.
...
[Quels sont les points qui vont le plus contribuer à la détermination des axes ?]{.rouge}
...
La [contribution relative de l’observation]{.rouge} $\class{alea}{x_i}$  à la construction de l’axe principal $k$  est la part d’inertie de l’axe $k$ expliquée par $\class{alea}{x_i}$ donnée par

$$Ctr(i, k) = \frac{1}{n} \frac{c_{ik}^2}{\lambda_k}.$$
En effet $\lambda_k = \frac{1}{n} \sum_{i=1}^n c_{ik}^2$ (ie. la variance du nuage projeté).
...
ggplot() +
  ...
  geom_text(aes(x = lambda1 * variables$xend[2], y = lambda1 * variables$yend[2]), label = expression(C^k == lambda_k~u^k),   col= "#d61d09", alpha = 0.5) +
  # geom_text(aes(x = -1.2, y = -2), label = expression(Delta), vjust = -0.8, size = 5) +
  ...
  geom_text(data = pos, aes(x = x, y = y), label = expression(theta["j,k"]), size = 5) +
  geom_text(data = pos, aes(x = x/3, y = 3*y), label = expression(theta["i,k"]), size = 5) +
  theme_minimal()  +
  coord_fixed() +  # Conserver un rapport d'aspect fixe pour les axes
  theme (axis.title=element_blank(),
         axis.text=element_blank(),
...
Ainsi $<X^j, C^k>$ est le $j^\text{ème}$ terme du vecteur $PC^\top C^k$, or $C^\top C$ est diagonale  avec $C_k^k = n lambda_k$, donc $C^\top C^k=(0, \dots, n\lambda_k, 0, \ldots)$
et

$$\cos(\theta_{jk}) = \frac{n \lambda_k \boldsymbol{u_k^j}}{\sqrt{\norm{X^j}^2 \, \norm{C_k}^2}}=  \sqrt{\lambda_k} \boldsymbol{u_k^j},  \quad \cos^2(\theta_{jk}) =  \lambda_k{(\boldsymbol{u_k^j})^2}$$
...
Si on souhaite ajouter une nouvelle variable $\boldsymbol{x}^{new}= (x^{new}_1, \ldots, x^{new}_n)^\top \in \R^n$
...
$$Cor(\tilde{\boldsymbol{x}}^{new}, u_k) = \frac{(\tilde{\boldsymbol{x}}^{new})^\top W \boldsymbol{u_k} }{\norm{\tilde{\boldsymbol{x}}^{new}}}$$
...
* Poids des individus ?
* Quelle distance entre les poids ?
* Lancer l'analyse 
...
On a utilisé l'ACP comme une technique de visualisation, mais on peut sen servir comme une technique de compression (avec perte).
Il suffit de ne garder que les première composantes principales.
