...
$$X^\top W X = P D P^\top.$$
* Les termes diagonaux de $D$ sont les [valeurs propres, toutes positives, ]{.rouge} de $X^\top W X$ rangées par odre décroissant, $\lambda_1 > \lambda_1 \ldots >\lambda_p$ et  $U$ est la matrice des vecteurs propres associés.
...
$$\cos^2(\theta_i) = \frac{(\class{alea}{x_i}^\top \boldsymbol{u_1})^2}{\norm{\class{alea}{x_i}}^2}$$
:::
...
ggplot() +
  ...
  geom_text(aes(x = lambda1 * variables$xend[2], y = lambda1 * variables$yend[2]), label = expression(C^k == lambda[k]~u^k),   col= "#d61d09", alpha = 0.5) +
  # geom_text(aes(x = -1.2, y = -2), label = expression(Delta), vjust = -0.8, size = 5) +
