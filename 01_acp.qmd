...
$X^\top X$ est la matrice de covariance, elle est [symétrique]{.rouge} et [définie positive]{.rouge} donc [diagonalisable]{.rouge}. Il existe une matrice $U$ unitaire $U^\top U = I_p,$ et $D$ une matrice diagonale telle que 
$$X^\top X = U D U^\top.$$

Les termes diagonaux de $D$ sont les [valeurs propres, toutes positives, ]{.rouge} de $X^\top X$ rangées par odre décroissant, $\lambda_1 > \lambda_1 \ldots >\lambda_p$ et  $U$ est la matrice des vecteurs propres associés.
...
Posons $\boldsymbol{v_1} = U^\top \boldsymbol{u_1},$  $\norm{\boldsymbol{v_1}}=1$:

Maximiser $\boldsymbol{v_1}^\top D \boldsymbol{v_1} = \sum_{k=1}^p v_{1p}^2 \lambda_p$, sous la contrainte $\sum_{k=1}^p  v_{1p}^2=1$

$$\boldsymbol{v_1} = (1, 0, \ldots, 0)^\top, \quad \boldsymbol{u_1} = U \boldsymbol{v_1} = \begin{pmatrix} U_{11} \\ \vdots \\U_{p1} \end{pmatrix}.$$
...
Posons $\boldsymbol{v_2} = U^\top \boldsymbol{u_2},$  $\norm{\boldsymbol{v_2}}=1$:
...
$$\boldsymbol{v_2} = (0, 1, \ldots, 0)^\top, \quad \boldsymbol{u_2} = U \boldsymbol{v_2} = \begin{pmatrix} U_{12} \\ \vdots \\U_{p2} \end{pmatrix}.$$
...
Réfléchissons aux données ce qui sera essentiel dans ce cours et dans la suite
...
Dans une ACP normée on s'intéresse donc aux valeurs propres de la matrice $X^\top M X.$
