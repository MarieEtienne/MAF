...
-   De quelles variables nous parle l'axe 1 ? l'axe 2 ? pour répondre à
    cette question on regarde le cercle des corrélations mais aussi les
...
-   Faites 2 ACP différentes sur chacun de ces deux groupess. Quelle est
    la première valeur propre dans les deux cas.

-   Rappelez en quoi la méthode mentionné plus haut est une ACP
    particulière. A partir de la fonction PCA et en utilisant l'option
    `col.w` mettez en oeuvre cette méthode.

-   On peut le faire directement, avec la fonction M̀FA \` de FactoMineR.
    Reprenez l'analyse précédente avec ce nouvel équilibreentre les
    différentes variables équilibrés. Quelles sont les informations que
    vous pouvez retenir ?
